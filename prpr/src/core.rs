...
thread_local! {
    ...
    /// 游戏内场景（开场动画 / 结算画面等）用的字体：assets/font.ttf。
    /// 界面（菜单、设置页）用主程序创建的默认 TextPainter（assets/harmonyos.ttf）。
    pub static GAME_FONT: RefCell<Option<TextPainter>> = RefCell::default();
}
