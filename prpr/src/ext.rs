...
/// 调试显示用：把原始 alpha 从 `[0, 1]` 线性重映射到 `[min_alpha, 1]` 再乘全局 alpha。
///
/// 这样本来会被隐藏 / 淡出的判定线与音符在调试模式下会以淡影留在屏幕上，
/// 越淡说明它越接近被隐藏（与参考实现一致）。
#[inline]
...
/// 用指定字体绘制对齐文字（例如游戏内场景统一用 [`crate::core::GAME_FONT`]）。
#[inline]
pub fn draw_text_aligned_using(
    ui: &mut Ui,
    font: &'static LocalKey<RefCell<Option<TextPainter>>>,
    text: &str,
    x: f32,
    y: f32,
    anchor: (f32, f32),
    ...
    ui.text(text).pos(x, y).anchor(anchor.0, anchor.1).size(scale).color(color).draw_using(font)
}

/// 超出 `max_width` 时自动缩小字号后再绘制（和参考实现的行为一致）。
pub fn draw_text_aligned_opt_width_using(
    ui: &mut Ui,
    font: &'static LocalKey<RefCell<Option<TextPainter>>>,
    text: &str,
    x: f32,
    ...
    let width = ui.text(text).size(scale).measure_using(font).w;
    if width > max_width && width > 0. {
        scale *= max_width / width;
    }
    draw_text_aligned_using(ui, font, text, x, y, anchor, scale, color)
}
