...
impl Resource {
    ...
    /// The colors of the challenge mode badge, in the order of `ChallengeColor`.
    ...
    pub async fn new(
        ...
        let health = crate::health::Health::new(&config);
        Ok(Self {
            config,
            info,
            aspect_ratio,
            ...
            res_pack,
            // 结算画面的默认头像沿用 Phi Recorder 的那张（phire 也是读 player.png）
