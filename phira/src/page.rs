...
static GLOBAL_ICONS: std::sync::Mutex<Option<std::sync::Arc<crate::icons::Icons>>> = std::sync::Mutex::new(None);

/// 存一份全局 UI 图标（HomePage 创建 Icons 时调用）。
/// 历史页这种没有 Arc<Icons> 的地方，要靠它来构造 SongScene。
pub fn set_global_icons(icons: std::sync::Arc<crate::icons::Icons>) {
    *GLOBAL_ICONS.lock().unwrap() = Some(icons);
}

/// 取全局 UI 图标。
pub fn global_icons() -> Option<std::sync::Arc<crate::icons::Icons>> {
    GLOBAL_ICONS.lock().unwrap().clone()
}
