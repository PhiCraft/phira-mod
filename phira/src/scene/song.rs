...
impl SongScene {
    ...
    pub fn global_launch(
        ...
        Ok(Some(Box::pin(async move {
            ...
            // 每日挑战：临时套用挑战约束（不写回保存的配置）
            if let Some(over) = crate::daily::take_pending_override() {
                config = over;
            }
            ...
            // 成绩上传：默认关。关掉时 upload_fn 为 None —— 引擎那边本来就是
            // `if let Some(upload_fn) = &self.upload_fn`，所以成绩只留在本地。
            let upload_fn: Option<UploadFn> = if get_data().config.upload_record {
    ...
    fn side_mods(&mut self, ui: &mut Ui, rt: f32) {
        ...
        self.mod_scroll.render(ui, |ui| {
            ...
            item(tl!("mods-full-screen-judge"), Some(tl!("mods-full-screen-judge-sub")), Mods::FULL_SCREEN_JUDGE);
            item(tl!("mods-health-mode"), Some(tl!("mods-health-mode-sub")), Mods::HEALTH_MODE);
            (width, h + 0.2)
        });
    }
    ...
    fn on_result(&mut self, tm: &mut TimeManager, res: Box<dyn Any>) -> Result<()> {
        let res = match res.downcast::<SimpleRecord>() {
            ...
            Ok(rec) => {
                ...
                // 成绩历史：本地记一条（服务端那边另有 rks/排行榜逻辑，互不影响）
                if rec.num_of_notes > 0 {
                    if let Err(err) = crate::history::record_play(
                        self.info.id,
                        self.local_path.as_deref(),
                        &self.info.name,
                        &self.info.level,
                        self.info.difficulty,
                        rec.score.max(0) as u32,
                        rec.accuracy as f64,
                        rec.max_combo,
                        rec.num_of_notes,
                        rec.counts,
                ...
                // 每日挑战：这一局算完成
                crate::daily::on_finished(rec.score.max(0) as u32, rec.accuracy as f64);
